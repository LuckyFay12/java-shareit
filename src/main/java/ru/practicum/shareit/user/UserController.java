...
public class UserController {
    ...
    @PatchMapping("/{userId}")
    public UserResponse update(@Min(1L) @PathVariable Long userId,
    ...
    public void delete(@Min(1L) @PathVariable Long userId) {
        log.info("Получен HTTP-запрос на удаление пользователя с id {}", userId);
        userService.delete(userId);
    }

    @GetMapping("/{userId}")
    public UserResponse get(@Min(1L) @PathVariable Long userId) {
